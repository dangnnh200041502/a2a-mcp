...
class AgentManager:
    ...
    async def _detect_calculator_intent(self, question: str) -> bool:
        """Use MCP client to detect if question contains arithmetic calculation."""
    ...
    async def _extract_calculation_expression(self, question: str) -> str:
        """Use MCP client to extract arithmetic expression from question."""
        await self.initialize_mcp()
        return await self._mcp_client.extract_calculation_expression(question)

    # ==========================
    # Decision
    # ==========================
    async def decide_tool(self, question: str, chat_history: List[Dict[str, Any]] | None = None) -> Dict[str, Any]:
        """
        ...
        if await self._mcp_client.detect_tool_intent(question, "weather"):
            return {"tool": "weather", "reason": "weather intent"}

        if await self._detect_calculator_intent(question):
    ...
    async def use_weather(self, question: str) -> Dict[str, Any]:
        """Use MCP weather tool."""
        await self.initialize_mcp()
        return await self._mcp_client.call_tool("weather", {"question": question})

    async def use_calculator(self, expression: str) -> Dict[str, Any]:
        """Use MCP calculator tool."""
        await self.initialize_mcp()
        return await self._mcp_client.call_tool("calculator", {"expression": expression})

    async def use_vector_search(self, query: str, chat_history: List[Dict[str, Any]] | None = None) -> Dict[str, Any]:
        """Use MCP vector search tool."""
        await self.initialize_mcp()
        return await self._mcp_client.call_tool("vector_search", {
            "query": query,
    ...
    async def orchestrate_answer(
        ...
        """Full agentic flow với planning & multi-tool orchestration using MCP."""
        ...
        for sub_q in candidates:
            ...
            if await self._mcp_client.detect_tool_intent(sub_q, "weather"):
                w = await self.use_weather(sub_q)
            ...
            elif await self._mcp_client.detect_tool_intent(sub_q, "calculator"):
                expr = await self._extract_calculation_expression(sub_q)
                ...
                rag = await self.use_vector_search(sub_q, chat_history)
